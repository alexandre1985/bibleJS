...
function* allVerseNumbers2File() {
	...
	//let maxVerse = yield getVerses(book, 1);
...
function arg2ChapterName(arg, beautiful) {
	...
	if(!isNaN(arg)) {
		...
		chapterName = beautiful2ChapterName(arg);
	}
	if(beautiful) {
		chapterName = chapterName.replace(/\-/g, ' ').trim();
		let lastSpace = chapterName.lastIndexOf(' ');
		chapterName = chapterName.substring(0, lastSpace) + ':' +  chapterName.substring(lastSpace+1)
	}
	return chapterName;
}
